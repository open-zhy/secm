...
func (m *Manager) LoadAll(cmd *cobra.Command) error {
	...
	for name, pluginPath := range m.registry.Plugins {
		...
		if err != nil {
			screen.Printf("WARN: failed to load plugin %s from %s: %v\n", name, fullPath, err)
			continue
		}

		initFn, err := plg.Lookup("Init")
		if err != nil {
			screen.Printf("WARN: plugin %s does not export Init symbol\n", name)
			continue
		}

		if init, ok := initFn.(Initializer); ok {
			init.Initialize(cmd)
			m.loaded[name] = plg
		} else {
			screen.Printf("WARN: plugin %s Init symbol is not a function\n", name)
		}
	}
